        ...
        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h4>Редактировать график работы</h4>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
                            ...
                            <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" 
                                            {{ old('user_id', $schedule->user_id) == $employee->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" 
                                      ...
                                      placeholder="Дополнительная информация о смене">{{ old('notes', $schedule->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <small class="text-muted">
                                <strong>Создано:</strong> {{ $schedule->creator->first_name }} {{ $schedule->creator->last_name }} 
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                ...
                                <button type="button" class="btn btn-danger ms-2" 
                                        onclick="if(confirm('Вы уверены, что хотите удалить этот график?')) { document.getElementById('delete-form').submit(); }">
                                    Удалить
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- Форма для удаления -->
                    <form id="delete-form" action="{{ route('admin.schedules.destroy', $schedule) }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
